...
void Gen_matrix(double A[], int m, int n, int thread_count);
void Read_matrix(char* prompt, double A[], int m, int n);
void Gen_vector(double x[], int n, int thread_count);
...
int main(int argc, char* argv[]) {
 ...
 # else
      Gen_matrix(A, m, n, thread_count);
/*    Print_matrix("We generated", A, m, n); */
      Gen_vector(x, n, thread_count);
...
void Gen_vector(double x[], int n, int thread_count) {
   ...
   {
      struct drand48_data *buffer = malloc(sizeof(struct drand48_data));
      srand48_r(omp_get_thread_num() * 50, buffer);
